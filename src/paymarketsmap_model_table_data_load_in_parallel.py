...
def process_company_paymarketsmap_model_data(input_company_id: int):
    try:
        # extract paymarketsmap model data
        company_id = int(input_company_id)
        logger.info(f"processing company id {company_id}")
        paymarketsmap_model_query = get_paymarketsmap_model_data_query(input_company_id=company_id)
        paymarketsmap_model_insert_json_procedure_query = get_paymarketsmap_model_insert_json_procedure_query()
        df_paymarketsmap_model_data = extract_and_process_model_table_data(company_id=company_id,
                                                                           query=paymarketsmap_model_query,
                                                                           connector=connector_read_source)
        insert_model_table_data(company_id=company_id, query=paymarketsmap_model_insert_json_procedure_query
                                , df=df_paymarketsmap_model_data, connector=connector_write_source, batch_size=batch_size)
        sleep = 0.5
        time.sleep(sleep)
        return company_id, True, len(df_paymarketsmap_model_data)
    except Exception as e:
        logger.exception(f"Error processing company id {input_company_id}: {e}")
        return input_company_id, False, str(e)
    # total_rows = total_rows + len(df_paymarketsmap_model)
...
with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
    futures = {executor.submit(process_company_paymarketsmap_model_data, int(row['Company_ID'])): int(row['Company_ID']) for
...
## step 4: rebuild the paymarketsmap model table index ##
logger.info(f"rebuilding the paymarketsmap model table index")
paymarketsmap_model_index_rebuild_query = get_paymarketsmap_model_index_rebuild_query()
rebuild_model_table_index(query=paymarketsmap_model_index_rebuild_query, connector=connector_write_source)
