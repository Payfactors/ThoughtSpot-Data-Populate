...
from sql_extract.sql_employees_model_queries import get_employees_model_data_query
from sql_extract.sql_employees_model_queries import get_employees_model_truncate_query
from sql_extract.sql_employees_model_queries import get_employees_model_delete_query
from sql_extract.sql_employees_model_queries import get_employees_model_index_rebuild_query
from sql_extract.sql_employees_model_queries import get_employees_model_insert_json_procedure_query
...
parser = argparse.ArgumentParser(
    description="This Python script accepts no input for all companies  and -c value for selected company.")
...
if input_company_id is None:
    logger.info(f"truncating the employees model table")
    employees_model_truncate_query = get_employees_model_truncate_query()
    truncate_model_table_data(query=employees_model_truncate_query, connector=connector_write_source)
else:
    logger.info(f"deleting the data for company : {input_company_id} in pricings model table")
    delete_query = get_employees_model_delete_query(input_company_id=int(input_company_id))
    delete_model_table_data(company_id=int(input_company_id), query=delete_query, connector=connector_write_source)

## step 3: load all employees model data into a variable
logger.info(f"Extracting and processing employees model data for all companies in parallel")


def process_company_employees_model_data(input_company_id: int):
    try:
        # extract employees model data
        company_id = int(input_company_id)
        logger.info(f"processing company id {company_id}")
        employees_model_query = get_employees_model_data_query(input_company_id=company_id)
        employees_model_insert_json_procedure_query = get_employees_model_insert_json_procedure_query()
        df_employees_model_data = extract_and_process_model_table_data(company_id=company_id,
                                                                       query=employees_model_query,
                                                                       connector=connector_read_source)
        # any columns that ends with _id should be converted to int
        for column in df_employees_model_data.columns:
            if column.upper() == 'FTE':
                df_employees_model_data[column] = pd.to_numeric(df_employees_model_data[column], errors='coerce').astype('Int64')
            if column.upper() == 'TENURE_YEARS_IN_SERVICE':
                df_employees_model_data[column] = pd.to_numeric(df_employees_model_data[column], errors='coerce').astype('Int64')
            if column.upper() == 'TENURE_MONTHS_IN_SERVICE':
                df_employees_model_data[column] = pd.to_numeric(df_employees_model_data[column], errors='coerce').astype('Int64')
        # insert employees model data
        insert_model_table_data(company_id=company_id, query=employees_model_insert_json_procedure_query
                                , df=df_employees_model_data, connector=connector_write_source, batch_size=batch_size)
        sleep = 0.5
        time.sleep(sleep)
        return company_id, True, len(df_employees_model_data)
    except Exception as e:
        logger.exception(f"Error processing company id {input_company_id}: {e}")
        return input_company_id, False, str(e)
    # total_rows = total_rows + len(df_employees_model)
...
with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
    futures = {executor.submit(process_company_employees_model_data, int(row['Company_ID'])): int(row['Company_ID']) for
...
## step 4: rebuild the employees model table index ##
logger.info(f"rebuilding the employees model table index")
employees_model_index_rebuild_query = get_employees_model_index_rebuild_query()
rebuild_model_table_index(query=employees_model_index_rebuild_query, connector=connector_write_source)
