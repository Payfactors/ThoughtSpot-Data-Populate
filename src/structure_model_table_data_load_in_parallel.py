...
from sql_extract.sql_structure_model_queries import get_structure_model_data_query
from sql_extract.sql_structure_model_queries import get_structure_model_truncate_query
from sql_extract.sql_structure_model_queries import get_structure_model_index_rebuild_query
from sql_extract.sql_structure_model_queries import get_structure_model_insert_json_procedure_query
...
## step 2: truncate the structure model table ##
logger.info(f"truncating the structure model table")
structure_model_truncate_query = get_structure_model_truncate_query()
truncate_model_table_data(query=structure_model_truncate_query, connector=connector_write_source)

## step 3: load all structure model data into a variable
logger.info(f"Extracting and processing structure model data for all companies in parallel")


def process_company_structure_model_data(input_company_id: int):
    try:
        # extract structure model data
        company_id = int(input_company_id)
        logger.info(f"processing company id {company_id}")
        structure_model_query = get_structure_model_data_query(input_company_id=company_id)
        structure_model_insert_json_procedure_query = get_structure_model_insert_json_procedure_query()
        df_structure_model_data = extract_and_process_model_table_data(company_id=company_id,
                                                                       query=structure_model_query,
                                                                       connector=connector_read_source)
        insert_model_table_data(company_id=company_id, query=structure_model_insert_json_procedure_query
                                , df=df_structure_model_data, connector=connector_write_source, batch_size=batch_size)
        sleep = 0.5
        time.sleep(sleep)
        return company_id, True, len(df_structure_model_data)
    except Exception as e:
        logger.exception(f"Error processing company id {input_company_id}: {e}")
        return input_company_id, False, str(e)
    # total_rows = total_rows + len(df_structure_model)
...
with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
    futures = {executor.submit(process_company_structure_model_data, int(row['Company_ID'])): int(row['Company_ID']) for
...
## step 4: rebuild the structure model table index ##
logger.info(f"rebuilding the structure model table index")
structure_model_index_rebuild_query = get_structure_model_index_rebuild_query()
rebuild_model_table_index(query=structure_model_index_rebuild_query, connector=connector_write_source)
