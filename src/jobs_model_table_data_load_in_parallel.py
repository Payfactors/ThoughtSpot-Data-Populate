...
from sql_extract.sql_job_model_queries import get_job_model_data_query
from sql_extract.sql_job_model_queries import get_job_model_truncate_query
from sql_extract.sql_job_model_queries import get_job_model_index_rebuild_query
from sql_extract.sql_job_model_queries import get_job_model_insert_json_procedure_query
...
## step 2: truncate the job model table ##
logger.info(f"truncating the job model table")
job_model_truncate_query = get_job_model_truncate_query()
truncate_model_table_data(query=job_model_truncate_query, connector=connector_write_source)

## step 3: load all job model data into a variable
logger.info(f"Extracting and processing job model data for all companies in parallel")


def process_company_job_model_data(company_id: int):
    try:
        # extract job model data
        logger.info(f"processing company id {company_id}")
        job_model_query = get_job_model_data_query(input_company_id=company_id)
        job_model_insert_json_procedure_query = get_job_model_insert_json_procedure_query()
        # load job model data into dataframe
        df_job_model_data = extract_and_process_model_table_data(company_id=company_id, query=job_model_query,
                                                                 connector=connector_read_source)
        # insert job model data dataframe into table in batches
        insert_model_table_data(company_id=company_id, query=job_model_insert_json_procedure_query, df=df_job_model_data
                                , connector=connector_write_source, batch_size=batch_size)
        sleep = 0.5
        time.sleep(sleep)
        return company_id, True, len(df_job_model_data)
    except Exception as e:
        logger.exception(f"Error processing company id {company_id}: {e}")
        return company_id, False, str(e)
    # total_rows = total_rows + len(df_job_model)
...
with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
    futures = {executor.submit(process_company_job_model_data, int(row['Company_ID'])): int(row['Company_ID']) for
...
## step 4: rebuild the job model table index ##
logger.info(f"rebuilding the job model table index")
job_model_index_rebuild_query = get_job_model_index_rebuild_query()
rebuild_model_table_index(query=job_model_index_rebuild_query, connector=connector_write_source)
